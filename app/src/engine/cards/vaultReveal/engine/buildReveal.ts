// Ported from card-pack-reveal-prototype/src/vault/engine/buildReveal.ts — card stack ->
// emergence -> fan -> rarity moment -> inspection. Everything is spring-driven toward per-phase
// targets, so inertia and settle come out of the simulation rather than being keyframed.
...
export function buildReveal(opts: BuildRevealOpts) {
  ...
  // Grail sits inside the fan, not on the end — it has to be noticed, not presented. Slot order
  // maps deck index -> left-to-right fan position. Generalised to any n (the original hardcoded
  // a 6-card order) so a pull whose item count ever differs from Vault Break's real spec (6)
  // degrades gracefully instead of indexing past the deck.
  const grailIdx = deck.findIndex((c) => c.rarity === "GRAIL");
  const order = buildSlotOrder(n, grailIdx);
  const slots = deck.map((_, i) => (order.indexOf(i) >= 0 ? order.indexOf(i) : i));

  // The fan's real full-spread width (outermost card centers, plus one card's own width) — the
  // prototype's camera distances for the "cards fanned out" phases were fixed constants tuned
  // on a wider preview than a portrait phone actually gives you; VaultScene uses this to pull
  // the camera back only as far as the real fan geometry (and the viewport's real aspect ratio)
  // needs, instead of a guess that clips on narrow screens.
  const spreadWidth = fan.stepX * (n - 1) + cardW;
  ...
  const cards: RevealCard[] = deck.map((data, i) => {
    ...
    // No castShadow here, deliberately — Tier 1's reveal has no per-card 3D meshes at all (its
    // cards are flat 2D swipe UI), so six independently-spring-animating shadow casters, each
    // needing its own shadow-depth-pass render every frame, is real added cost this pass has no
    // equivalent for elsewhere in the app. The cards read fine on their own emissive/metalness
    // material without also casting shadows onto the pack/floor below them.
...
/** The original hardcoded `[0, 3, 1, grailIdx, 2, 4]` for exactly 6 cards — generalised here so
 * an unexpected item count degrades to a plain left-to-right order instead of throwing. For the
 * real n=6 case with a grail, this reproduces the exact same slot layout (grail's own index
 * still lands in the same "third from left" position the original design placed it at). */
function buildSlotOrder(n: number, grailIdx: number): number[] {
  if (n !== 6) {
    const order = Array.from({ length: n }, (_, i) => i);
    if (grailIdx >= 0) {
      const mid = Math.floor((n - 1) / 2);
      order.splice(order.indexOf(grailIdx), 1);
      order.splice(mid, 0, grailIdx);
    }
    return order;
  }
  return [0, 3, 1, grailIdx < 0 ? 4 : grailIdx, 2, 4];
}
