// Ported from card-pack-reveal-prototype/src/vault/gesture/useVaultInteraction.ts — the full
// pointer state machine from the original web prototype: tear-the-seam, then (once sealed ->
// open) tap-to-inspect / drag-to-rotate / flip / orbit, re-based onto r3f's own raycasted
// pointer events (one event system per view, not a second gesture responder layered on top).
//
// Differences from the original's mouse/wheel-shaped controls:
//   - `wheel` (desktop-only zoom of the inspected card) has no mobile equivalent, so two-finger
//     pinch drives `reveal.zoomHero` instead.
//   - `dblclick` (flip the inspected card) becomes a same-card double-tap, detected here since
//     RN has no native double-tap event on r3f's pointer stream.
//   - free orbit (the "Orbit" pill) used three.js's OrbitControls addon, which is DOM-only.
//     Single-finger drag drives a small hand-rolled spherical orbit here instead; pinch dollies
//     the camera in/out.
...
const FLICK_THRESHOLD = 3.0; // fraction-of-seam per second
...
export function useVaultInteraction(pack: BuiltVaultPack, personality: VaultBreakPersonality) {
  ...
  const onPointerDown = (pointerId: number, localXY: { x: number; y: number }, planeXY: { x: number; y: number }, ray: THREE.Ray) => {
    ...
    if (s.pointers.size === 2) {
      // second finger down mid-interaction: hand off to pinch (zoom the hero card if one is up,
      // otherwise ignored)
      s.pinchPids = Array.from(s.pointers.keys());
      s.pinchStartDist = pinchDistance(s);
      s.tearPid = null;
  ...
  const onPointerMove = (pointerId: number, localXY: { x: number; y: number }, planeXY: { x: number; y: number }) => {
    ...
    // Orbit-rotate and hero-card-rotate deltas are computed by the caller (scene/VaultScene.tsx)
    // from its own before/after plane position and applied via dragDelta() — this handler only
    // needs to update the bookkeeping above (for pinch distance) and drive the tear.
  ...
  /** Screen-fraction-ish drag delta for orbit rotate / hero rotate, computed by the caller from
   * the interaction plane's own local space (see VaultScene.tsx) so it never depends on
   * ambiguous native pointer fields. */
