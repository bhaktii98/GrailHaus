// Ported from card-pack-reveal-prototype/src/vault/art/artUtil.ts — shared drawing helpers
// between the pack art (vaultArt.ts) and the card art (cardArt.ts). Built on top of
// ../../reveal/art/canvasHelpers.ts's Paint/Path/Shader vocabulary rather than duplicating it —
// metalFill, hairlines, grooveLine, guilloche, microBlock, barcode and embossText are the
// primitives Tier 1's flatter, chrome-not-satin art didn't need.
import {
  Skia,
  ClipOp,
  type SkCanvas,
...
/** Vertical brushed-metal gradient shader. `stops` overrides the default champagne ramp so the
 * one helper serves gold, plum, or graphite. */
...
/** Embossed type: a dark bed below, a lit rim above, then the metal face — reads as pressed into
 * the substrate rather than printed on it. */
...
/** Micro-engraved hairlines. Very low contrast on purpose: at pack scale this reads as a satin
 * machine finish, not as stripes. `angle` in radians. */
...
/** Guilloche rosette — the interference-pattern linework used on banknotes and certificates.
 * Pure strokes, so it stays quiet at small sizes. */
