...
// Emergence with a deliberate hold at half height: the card stops, the fire
// keeps moving, then it continues. The pause is the point.
const pauseEase = (t) => (t < 0.44 ? ease(t / 0.44) * 0.5
  : t < 0.63 ? 0.5
    : 0.5 + ease((t - 0.63) / 0.37) * 0.5);
...
export function buildReveal(opts) {
  ...
  const CARD_T = (cfg.card && cfg.card.thickness) || 0.0006;
  const BEVEL = (cfg.card && cfg.card.bevel) || 1.0;

  const cards = deck.map((data, i) => {
    const g = new THREE.Group();
    g.name = `card${i}_${data.rarity}`;
    const faceMat = new THREE.MeshStandardMaterial({
      name: `cardFace${i}`, map: faceTexes[i],
      roughness: 0.3 - (data.metalBoost || 0) * 0.06,
    ...
    face.position.z = CARD_T / 2 + 0.00004;
    face.castShadow = true;
    const verso = new THREE.Mesh(geo, versoMat);
    verso.name = `cardVersoMesh${i}`;
    verso.position.z = -(CARD_T / 2 + 0.00004);
    verso.rotation.y = Math.PI;

    // Card body: real thickness with a dark metallic bevel standing a hair
    // proud of the printed faces, so every edge catches its own highlight.
    const rimMat = new THREE.MeshStandardMaterial({
      name: `cardRim${i}`,
      color: data.rarity === 'GRAIL' ? 0x120c0a : 0x0b0b0f,
      roughness: 0.24, metalness: 0.94,
      emissive: new THREE.Color(data.rarity === 'GRAIL' ? 0xff5a12 : 0x1a0f08),
      emissiveIntensity: 0,
    });
    const rim = new THREE.Mesh(
      new THREE.BoxGeometry(cardW * BEVEL, cardH * BEVEL, CARD_T),
      rimMat,
    );
    rim.name = `cardBody${i}`;
    rim.castShadow = true;
    g.add(rim, face, verso);
    ...
    group.add(g);
    return {
      data, obj: g, faceMat, versoMat, rimMat, hit, index: i, slot: slots[i],
    ...
    if (c.isGrail && !state.grailShown) {
      ...
      if (state.phase === 'approach' || state.phase === 'reveal') {
        const q = state.phase === 'reveal'
          ? pauseEase(clamp(phaseT / timing.reveal))
  ...
  const step = (dt) => {
    if (state.running) {
      ...
      if (state.phase === 'reveal' && phaseT > timing.reveal * 0.7) state.grailShown = true;
    }
    ...
    cards.forEach((c) => {
      ...
      // glowing edges: the bevel is where the card's own heat shows
      const rimWant = c.isGrail
        ? 0.35 + focusBoost * 1.9 + (state.grailShown ? 0.5 : 0)
        : 0.04 + focusBoost * 0.3;
      c.rimMat.emissiveIntensity += (rimWant - c.rimMat.emissiveIntensity) * Math.min(1, dt * 2.4);
    });
  ...
  return {
    ...
    grailIndex: grailIdx,
    get grailCard() { return grailIdx >= 0 ? cards[grailIdx] : null; },
