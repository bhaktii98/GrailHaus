...
export const SIZE = TIERS['street-rip'].size;
...
export function buildPack(logo, cfg = TIERS['street-rip']) {
  const { W, H, T } = cfg.size;
  const SEAM_FRAC = cfg.seamFrac, FLAP_FRAC = cfg.flapFrac;
  ...
  const frontTex = tex(cfg.art.front(logo));
  const backTex = tex(cfg.art.back());
  const glintTex = tex(cfg.art.shine());
  const cardTex = cfg.art.cardBack ? tex(cfg.art.cardBack()) : null;
  ...
  const foil = (map, name) => new THREE.MeshStandardMaterial({
    name, map, roughness: cfg.material.roughness, metalness: cfg.material.metalness,
    flatShading: true,
  ...
  // ---- inner liner (tier 2+): metal web inside the mouth, dark interior --
  const liner = new THREE.Group();
  liner.name = 'packLiner';
  let linerFront = null, linerBack = null;
  if (cfg.liner && cfg.liner.enabled && cfg.art.liner) {
    const linerTex = tex(cfg.art.liner());
    const lmat = new THREE.MeshStandardMaterial({
      name: 'innerLiner', map: linerTex,
      metalness: cfg.liner.metalness, roughness: cfg.liner.roughness,
      side: THREE.DoubleSide,
    });
    // The liner is a shrunken copy of each wall: same profile, 68% of its
    // depth, so it can never push through the printed foil. Top edge sits at
    // the seam, never above it.
    const lh = H * 0.28;
    const mk = (sign, name) => {
    ...
    const interior = new THREE.Mesh(
      new THREE.PlaneGeometry(W * 0.96, H * 0.42),
      new THREE.MeshStandardMaterial({ name: 'interiorShell', color: 0x06030d, roughness: 0.92, metalness: 0 }),
    );
  ...
  let cards, reveal = null;
  if (cfg.revealProfile === 'premium') {
    const deck = cfg.deck();
    const faceTexes = deck.map((c) => tex(cfg.cardArt.face(c)));
    const versoTex = tex(cfg.cardArt.verso());
    reveal = buildReveal({
      cfg, deck, faceTexes, versoTex, cardW: cw, cardH: ch,
      restY: cardRestY, packT: T,
    });
    cards = reveal.group;
  } else {
  ...
  const PEEL = cfg.material.peel;

  // Foil stretch under the thumb before the tear front starts moving. Reads
  // as material density; costs the user only a few pixels of travel.
  let stretchAmt = 0, stretchU = 0;
  const setStretch = (amount, u) => { stretchAmt = amount; stretchU = u; };
  ...
  const setProgress = (p) => {
    ...
    if (reveal) {
      ...
      cards.children.forEach((c, i) => {
        ...
        c.position.set(
          k * fanQ * W * 0.16,
          cardRestY + rq * cfg.riseY - fanQ * Math.abs(k) * 0.004,
          (i - 1) * 0.0013,
        );
  ...
  const react = (dt, pullX, pulling) => {
    ...
    stepPhys(dt);
    if (reveal) reveal.step(dt);
  };

  const setTime = (time, energy = 0) => {
    glintTex.offset.x = (time * (0.11 + energy * 0.45)) % 1;
    const idle = 0.3 + 0.14 * Math.sin(time * 1.3);
    glintMat.opacity = fade * Math.min(1.1, idle + energy * 0.6) * (cfg.tear.glint ?? 1);
  };

  setProgress(0);
  return {
    group, cfg, size: cfg.size, seamY, setProgress, setTime, react,
    setStretch, reveal, cards, cardRestY, cardSize: { w: cw, h: ch },
  };
}
